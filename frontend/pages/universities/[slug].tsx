...
export async function getStaticPaths() {
  ...
  try {
    const res = await fetch(`${backendUrl}/api/data/university-slugs`);
...
export async function getStaticProps({ params }) {
  ...
  try {
    const res = await fetch(`${backendUrl}/api/data/universities/${slug}`);
    if (!res.ok) return { notFound: true, revalidate: 3600 };
    ...
    const students = (data.members || []).map((s) => {
    ...
    return { props: { university: data.university, students }, revalidate: 3600 };
  } catch (err) {
    console.error("[university getStaticProps]", err);
