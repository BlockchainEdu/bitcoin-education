...
export default function HeaderWithLogo({ className = "", children }) {
  ...
  return (
    <section
      className={`py-10 px-7 sticky top-0 z-10 white-header header ${
        offset > 100 ? "scrolled" : ""
      } ${className}`}
    >
      <Head>
      ...
      <nav className="flex max-w-7xl m-auto justify-between items-center relative">
        <ul
          className={`font-mont text-black transition duration-500 w-5/12 ${className}`}
        >
          <li className="flex gap-x-14">
            <a
              className="hover:text-benorange-500 transition duration-500"
              href="/"
            >
              Home{" "}
            </a>
        ...
        <div className="w-2/12 lg:w-1/3 absolute lg:relative top-[-.5rem] lg:top-0 left-0">
          {" "}
          <a href="/">
            <img className="w-24 mx-auto" src={logoSrc} />
          </a>{" "}
        </div>
        <ul className={`font-mont text-black w-5/12 ${className}`}>
          <li className="flex gap-x-14 justify-end items-center">
            <a
              className="hover:text-benorange-500 transition duration-500"
              target="_blank"
              href="https://learn.blockchainedu.org/events"
            >
              Events{" "}
            </a>
            <a
            ...
            <a
              className="hover:text-benorange-500 transition duration-500 font-bold"
              href="/contact"
            >
              Contact{" "}
            </a>
            <a
              id="always-visible-menu-button"
              target="_blank"
              href="https://learn.blockchainedu.org/share/pngyhAckxcbUqhlg?utm_source=manual"
            >
