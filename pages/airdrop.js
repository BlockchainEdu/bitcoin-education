...
export default function Airdrop() {
  ...
  useEffect(() => {
    async function fetchLeaderboardData() {
      ...
      try {
        const response = await axios.post("/api/monday", body);
        ...
        if (response.data && response.data.data && response.data.data.boards) {
          ...
          let sortedTeams = Object.keys(teamCounts)
            .map((team) => ({
              name: team,
  ...
  const addEntryToMondayBoard = async (email, ethAddress, team) => {
    ...
    try {
      const response = await axios.post("/api/monday", { query });
    ...
    return (
      <tr
        ...
        className={`clickable-row ${
          isSponsoringRow ? "bg-yellow-100-important text-black" : ""
        }`}
        onClick={() =>
          isSponsoringRow ? null : window.open(entry.url, "_blank")
        }
      >
  ...
  const renderLeaderboardTable = () => (
    <div className="leaderboard-container">
      <table className="min-w-full">
        ...
        <tbody>{leaderboardData.map(renderLeaderboardRow)}</tbody>
      </table>
    </div>
  );
    ...
    <div id="partners-page">
      ...
      <div id="team-page">
        <div className="countdown-section bg-black text-white p-10 text-center">
          ...
          <p>
            <Countdown deadline={deadline} /> until liftoff
          </p>
          <p>
            Mark your calendars for our Twitter Spaces on Jan 31st where we will
            be doing the airdrop live
          </p>
        </div>
        ...
        <div className="countdown-section bg-yellow-100 text-black p-10 text-center">
          ...
          <div className="grid grid-cols-2 gap-4 items-center">
            <img
              src="/images/airdrop/life-wallet.jpg"
              alt="Life Wallet"
              className="max-w-[100px]"
            />
            <img
              src="/images/airdrop/gda.jpeg"
              alt="GDA"
              className="max-w-[100px]"
            />
          </div>
...
*/}
        {/* Leaderboard Table */}
        <div className="bg-black">{renderLeaderboardTable()}</div>
      </div>
      <Footer />
    </div>
  );
