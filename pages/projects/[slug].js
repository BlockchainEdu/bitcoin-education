              ...
              {project.gallery?.map((item, idx) => (
                <SwiperSlide className="w-4/5 pb-24">
                  {item.file_extension === 'youtube' && item.public_url != '' && idx === currSlideIdx &&
                   <YouTube videoId={item.public_url.split("/").pop()} opts={{playerVars: { autoplay: 1 }}} containerClassName="aspect-photo flex justify-center items-center swiper-slide-vimeo" />
                  }
                  {item.file_extension === 'vimeo' && item.public_url != '' && idx === currSlideIdx &&
                    <Vimeo video={item.public_url} className="aspect-photo flex justify-center items-center swiper-slide-vimeo" autoplay />
                  }
                  {item.file_extension === '.jpg' && item.public_url != '' && idx === currSlideIdx &&
                    <div className="aspect-photo mx-auto">
